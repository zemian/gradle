...
import org.gradle.play.integtest.continuous.AbstractPlayReloadIntegrationTest
import spock.lang.Unroll
/**
 * Test that app requests block while a build is in progress when using `--continuous`.
 */
class PlayExternalContinuousBuildReloadWaitingIntegrationTest extends AbstractPlayReloadIntegrationTest {
    ...
    String getDefaultBuildFileName() {
        "build-external.gradle"
    }

    def setup() {
        settingsFile << """
            rootProject.buildFileName = "build-external.gradle"
        """
        server.start()
